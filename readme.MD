...
Copiar código
npm start
El servidor se ejecutará en el puerto 8080 por defecto. Puedes acceder a la aplicación en tu navegador web en http://localhost:8080.
...
bash
Copiar código
curl -X GET "http://localhost:8080/api/products?limit=5&page=1&sort=asc&query=category=electronics"
...
bash
Copiar código
curl -X GET "http://localhost:8080/api/products/60c72b2f5f1b2c001c8d4c50"
...
Cuerpo de la Solicitud: (Cualquier campo del producto que desees actualizar)
...
curl -X PUT -H "Content-Type: application/json" -d '{
  "price": 120.0
}' "http://localhost:8080/api/products/60c72b2f5f1b2c001c8d4c50"
...
bash
Copiar código
curl -X DELETE "http://localhost:8080/api/products/60c72b2f5f1b2c001c8d4c50"
Carrito de Compras
Ver el Carrito
Endpoint: /api/carts
...
curl -X POST -H "Content-Type: application/json" -d '{
  "quantity": 2
}' "http://localhost:8080/api/carts/add/60c72b2f5f1b2c001c8d4c50"
...
curl -X POST -H "Content-Type: application/json" -d '{
  "quantity": 1
}' "http://localhost:8080/api/carts/remove/60c72b2f5f1b2c001c8d4c50"
Vaciar el Carrito
...
Contribuciones
Las contribuciones son bienvenidas. Por favor, abre un issue o envía un pull request si tienes mejoras o nuevas funcionalidades que te gustaría agregar.

¡Gracias por usar nuestra plataforma de Ecommerce! Si tienes alguna pregunta o necesitas ayuda, no dudes en contactarnos.

r
Copiar código

Este README proporciona una guía completa para clonar el proyecto, instalar las dependencias, utilizar `npm start`, y detalla los endpoints disponibles junto con ejemplos de cómo usarlos.